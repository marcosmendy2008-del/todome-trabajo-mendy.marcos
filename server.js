    ...
    app.get('*', (req, res) => {
        ...
        res.sendFile(indexPath, (err) => {
            if (err) {
                ...
                // Si falla al buscar la carpeta, intentamos desde la raíz relativa
                res.sendFile(path.join(process.cwd(), 'frontend', 'index.html'), (err2) => {
                    if (err2) {
                        res.status(500).send("No se encontró el archivo index.html en el servidor.");
                    }
                });
            }
        });
